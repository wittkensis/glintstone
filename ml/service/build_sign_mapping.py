    ...
    with open(ebl_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if len(parts) >= 3:
                ...
                mapping[sign_name] = {"abz": abz_code, "unicode": unicode_char}
    return mapping
...
def build_class_mapping(checkpoint_path: Path, ebl_mapping: dict) -> list:
    ...
    for i, name in enumerate(class_names):
        entry = {"id": i, "name": name, "unicode": None, "abz": None}
...
def main():
    ...
    ebl_path = Path(
        "/Volumes/Portable Storage/CUNEIFORM/downloads/ebl-annotations/cuneiform_ocr_data/sign_mappings/ebl.txt"
    )
