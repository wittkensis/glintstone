---
question: "How does Glintstone's citation pipeline pull publication and edition data together from nine external sources?"
created: 2026-05-11
modified: 2026-05-11
context: "Created during the schema design phase to summarize the citation pipeline's design impact on the data model. Moved into docs/data-model/ during the 2026-05-11 knowledge architecture overhaul as a narrative companion to the schema YAML."
status: active
audience: [engineers]
owners: [eric]
related_issues: []
...
This document describes the citation sourcing pipeline built to populate the trust, citation, knowledge graph, and annotation infrastructure in the Glintstone schema. Written for handoff to another agent reviewing the data model.
...
### Pipeline-Internal Table (not in schema)
...
|-------|---------------|-----|
| `artifact_identifiers` | Seeded from artifacts.museum_no/excavation_no during migration, not by this pipeline | Different migration scope |
...
2. **publications.publication_type**: The schema defines 9 values. The pipeline maps from CDLI (numeric entry_type_id), OpenAlex (CSL types), eBL (CSL types), and BibTeX (entry types). Some mappings are approximate. Should we add "digital_edition" handling for ORACC projects or is that covered?
