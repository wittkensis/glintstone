...
Usage:
    python 03_ebl_bibliography.py [--reset] [--verify-only] [--token TOKEN]
"""
...
class EBLBibliographyImporter:
    ...
    def __init__(self, api_token: str | None = None, reset: bool = False):
    ...
    def _get_ebl_fragments(self, conn: psycopg.Connection) -> list[tuple[str, str]]:
        """
        ...
        try:
            for record in batch:
                ...
                if match.publication_id:
                    ...
                    cursor.execute(
                        "SELECT id FROM publications WHERE bibtex_key = %s", (bk,)
                    )
                    row = cursor.fetchone()
...
def main():
    ...
    importer = EBLBibliographyImporter(api_token=args.token, reset=args.reset)
