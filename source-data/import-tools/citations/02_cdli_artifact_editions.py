...
Usage:
    python 02_cdli_artifact_editions.py [--reset] [--csv-only] [--verify-only]
"""
...
class CDLIArtifactEditionImporter:
    ...
    def __init__(self, reset: bool = False, csv_only: bool = False):
    ...
    def _import_from_api(self, conn: psycopg.Connection):
    ...
    def _import_from_csv(self, conn: psycopg.Connection):
    ...
    def _resolve_publication(self, designation: str, year: str | None) -> int | None:
        """Match a designation to an existing publication via publication_matcher."""
        candidate = {
            "short_title": designation,
            "year": year,
            "bibtex_key": designation.replace(" ", ""),
        }
        result = find_match(candidate, self._conn)
        if result.publication_id and result.confidence >= 0.7:
            return result.publication_id
        return None

    def _stage_unparsed(self, p_number: str, raw_text: str, tier: str, reason: str):
        """Write unparseable record to _unparsed_records staging table."""
                ...
                cursor.execute(
                    """INSERT INTO artifact_editions
                       ...
                       ON CONFLICT(p_number, publication_id, reference_string) DO UPDATE SET
                           confidence = GREATEST(excluded.confidence, artifact_editions.confidence),
                           note = COALESCE(artifact_editions.note, excluded.note)""",
                    (
                        record["p_number"],
                        pub_id,
...
def main():
    ...
    importer = CDLIArtifactEditionImporter(reset=args.reset, csv_only=args.csv_only)
