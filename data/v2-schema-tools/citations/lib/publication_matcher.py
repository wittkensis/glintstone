...
def find_match(
    ...
    Args:
        ...
        conn: SQLite connection to v2 database
    ...
    if doi:
        cursor.execute(
            "SELECT id, bibtex_key FROM publications WHERE doi = ?",
            (doi,),
        )
        row = cursor.fetchone()
        if row:
    ...
    if bk:
        cursor.execute(
            "SELECT id, bibtex_key FROM publications WHERE bibtex_key = ?",
            (bk,),
        )
        row = cursor.fetchone()
        if row:
    ...
    if title and year:
        ...
        cursor.execute(
            "SELECT id, bibtex_key, title FROM publications WHERE year = ?",
            (str(year),),
        )
        best_ratio = 0.0
        best_row = None
    ...
    if short_title and volume:
        cursor.execute(
            ...
            "WHERE short_title = ? AND volume_in_series = ?",
...
def record_dedup_candidate(
    ...
    conn.execute(
        """INSERT OR IGNORE INTO _dedup_candidates
...
def ensure_dedup_table(conn: sqlite3.Connection):
