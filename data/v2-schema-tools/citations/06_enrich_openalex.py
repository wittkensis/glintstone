...
Usage:
    python 06_enrich_openalex.py [--reset] [--verify-only] [--db PATH]
"""
...
class OpenAlexEnricher:
    ...
    def _backfill_dois(self, conn: sqlite3.Connection, works: list) -> int:
    ...
    def _backfill_orcids(self, conn: sqlite3.Connection, works: list) -> int:
        ...
        for scholar_id, name in scholars:
            ...
            if orcid:
                try:
                    cursor.execute(
                        "UPDATE scholars SET orcid = ? WHERE id = ?",
                        (orcid, scholar_id),
                    )
                    count += 1
                except sqlite3.IntegrityError:
                    pass  # ORCID already assigned to another scholar

        conn.commit()
        self.checkpoint.stats["updated"] += count
        return count

    def _import_new_publications(self, conn: sqlite3.Connection, works: list) -> int:
...
def verify(db_path: Path):
    ...
    c.execute(
        ...
        "WHERE ar.source_name = ?", (PROVIDER_NAME,)
    )
...
def main():
    ...
    enricher = OpenAlexEnricher(args.db, reset=args.reset)
