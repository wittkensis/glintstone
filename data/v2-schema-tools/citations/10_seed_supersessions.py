...
Usage:
    python 10_seed_supersessions.py [--verify-only] [--db PATH]
"""
...
def seed_supersessions(db_path: Path):
    ...
    for current_title, supersedes_title, scope in KNOWN_SUPERSESSIONS:
        ...
        cursor.execute(
            "SELECT id FROM publications WHERE short_title = ?",
            (current_title,),
        )
        ...
        if current and superseded:
            cursor.execute(
                "UPDATE publications SET supersedes_id = ?, superseded_scope = ? WHERE id = ?",
