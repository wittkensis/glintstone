...
Usage:
    python 04_oracc_editions.py [--reset] [--verify-only] [--db PATH]
"""
...
ORACC_DIR = Path("/Volumes/Portable Storage/Glintstone/data/sources/ORACC")
...
class ORACCEditionImporter:
    ...
    def _ensure_annotation_run(self, conn: sqlite3.Connection, provider_name: str) -> int:
        cursor = conn.cursor()
        cursor.execute(
    ...
    def _register_project_publication(self, conn: sqlite3.Connection, project_key: str, run_id: int) -> int:
        ...
        cursor.execute(
            "SELECT id FROM publications WHERE bibtex_key = ?", (bibtex_key,)
        )
        return cursor.fetchone()[0]

    def _extract_scholars(self, conn: sqlite3.Connection, project_key: str, project_dir: Path, pub_id: int):
    ...
    def _upsert_scholar(self, conn: sqlite3.Connection, name_raw: str, pub_id: int):
        """Insert scholar and link to publication."""
        cursor = conn.cursor()

        cursor.execute(
            "INSERT INTO scholars (name) VALUES (?) ON CONFLICT DO NOTHING",
            (name_raw,),
        )
        cursor.execute("SELECT id FROM scholars WHERE name = ?", (name_raw,))
        row = cursor.fetchone()
        if not row:
            return
...
def main():
    ...
    importer = ORACCEditionImporter(args.db, reset=args.reset)
