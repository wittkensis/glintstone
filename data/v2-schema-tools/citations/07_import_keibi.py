...
Usage:
    python 07_import_keibi.py [--reset] [--verify-only] [--db PATH]
                              [--bibtex-dir DIR]
"""
...
DEFAULT_BIBTEX_DIR = Path("/Volumes/Portable Storage/Glintstone/data/sources/KeiBi")
...
class KeiBiImporter:
    ...
    def __init__(self, db_path: Path, bibtex_dir: Path, reset: bool = False):
    ...
    def _import_entries(self, conn: sqlite3.Connection, entries: list[dict]) -> int:
...
def main():
    ...
    importer = KeiBiImporter(args.db, args.bibtex_dir, reset=args.reset)
