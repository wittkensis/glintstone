...
# PYTHON VERSION PIN (BUG-4; direction reversed 2026-09-05, task #1419).
#
# The target runtime is Python 3.12 — NOT 3.13. The VPS runs Alpine Linux v3.22,
# whose only packaged interpreter is python3 3.12.x (`apk policy python3` offers
# 3.12.12/3.12.13 and nothing newer). Getting 3.13 onto that box would mean
# pulling from Alpine edge or building CPython from source on a live production
# host — a real, risky runtime change — and the codebase gains nothing from it:
# a sweep of every .py file found no 3.13-only syntax or stdlib (no typing.Self,
# no except*, no TypeIs/@override/copy.replace/PythonFinalizationError). So the
# cheap, safe reconciliation is to pin the repo DOWN to prod, not to drag prod up.
#
# The original BUG-4 fix went the other way — it preferred an explicit
# `python3.13` and hard-aborted below 3.13. On this VPS that gate could never
# pass, so it would have blocked every deploy. It is now inverted: prefer the
# newest 3.12+ interpreter available, and gate on >= 3.12. The gate is kept (not
# deleted) because its real job is catching SILENT interpreter drift — it just
# has to compare against the version we actually target.
#
# If the floor ever moves, three things move together: this gate,
# `.python-version` at the repo root, and the `python-version` in
# .github/workflows/{test,deploy}.yml.
echo "[$(date -u +%H:%M:%S)] Installing dependencies into release venv (timeout 8m)..."
ssh_run "timeout 480 sh -c 'cd $RELEASE_DIR && { command -v python3.13 >/dev/null 2>&1 && PYBIN=python3.13 || { command -v python3.12 >/dev/null 2>&1 && PYBIN=python3.12 || PYBIN=python3; }; } && \"\$PYBIN\" -m venv venv && venv/bin/pip install --quiet --upgrade pip && venv/bin/pip install --quiet -r requirements.txt'" || {
...
# Refuse to promote a release whose venv Python is older than 3.12. This is the
# backstop that turns silent version drift into a loud, blocking failure.
echo "[$(date -u +%H:%M:%S)] Verifying release venv Python is >= 3.12..."
ssh_run "cd $RELEASE_DIR && venv/bin/python -c 'import sys; v=sys.version_info; print(\"venv python:\", \".\".join(map(str, v[:3]))); sys.exit(0 if v[:2] >= (3, 12) else 1)'" || {
    echo "::error::Release venv Python is older than 3.12 (BUG-4) — aborting. Install python3.12 on the VPS, then re-run."
    exit 1
}
